...
void parse_command_line(const char* command_line, enum command* command_out, char*** args_out, int* args_size_out) {
    int command_size = 0;
    while (command_line[command_size] != ' ') {
        if (command_line[command_size] == '\0' || command_size > 127)
            return;
...
char* read_command_line() {
    ...
    while (scancode != 0x1C) { // Enter
        ...
        if (is_character(scancode)) {
            char ascii = scancode_to_ascii[scancode];
            if ((ascii >= 'A' && ascii <= 'Z') || (ascii >= 'a' && ascii <= 'z') || (ascii == ' '))
...
void main(void) {
    ...
    while (1) {
        ...
        char** args;
        int args_size;
        parse_command_line(command_line, &command, &args, &args_size);
